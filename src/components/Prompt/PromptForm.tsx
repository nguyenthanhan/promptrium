...
const PromptForm: React.FC<PromptFormProps> = ({ prompt, onSubmit, onCancel, isLoading = false }) => {
      ...
      <div className="space-y-2">
        ...
        <div className="flex space-x-2">
          <input
            ...
            onKeyPress={(e) => (e.key === 'Enter' || e.key === ',') && (e.preventDefault(), handleAddTag())}
            ...
            disabled={isLoading}
          />
          <Button type="button" variant="secondary" size="sm" onClick={handleAddTag} disabled={!newTag.trim() || isLoading}>
            <Plus className="w-4 h-4" />
          </Button>
        </div>
        
        <p className="text-xs text-gray-500">Press Enter or comma to add a tag. Click the X to remove a tag.</p>
      </div>
